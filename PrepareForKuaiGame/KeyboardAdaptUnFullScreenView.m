...
- (void)adjustTextFieldLayoutWithIsShowingKeyboard:(BOOL)isShowingKeyboard {
    ...
    if (isIOS8) {
        if (isShowingKeyboard) {
            double keyboardTopY = [[UIApplication sharedApplication].keyWindow convertPoint:CGPointMake(0, CGRectGetHeight([UIScreen mainScreen].bounds) - CGRectGetHeight(self.keyboardFrame)) toView:self].y;
            textFieldFrame.origin.y = keyboardTopY - CGRectGetHeight(textFieldFrame);
            self.textField.frame = textFieldFrame;
        } else {
            double keyboardTopY = [[UIApplication sharedApplication].keyWindow convertPoint:CGPointMake(0, CGRectGetHeight([UIScreen mainScreen].bounds)) toView:self].y;
            textFieldFrame.origin.y = keyboardTopY;
            self.textField.frame = textFieldFrame;
        }
    } else {
        __weak typeof(self) weakSelf = self;
        ...
        if (isShowingKeyboard) {
            switch ([UIDevice currentDevice].orientation) {
                case UIDeviceOrientationPortrait:
                    srcPoint = CGPointMake(0, CGRectGetHeight([UIScreen mainScreen].bounds) - CGRectGetHeight(self.keyboardFrame));
                    width = CGRectGetWidth([UIScreen mainScreen].bounds);
                    break;
                case UIDeviceOrientationPortraitUpsideDown:
                    srcPoint = CGPointMake(CGRectGetWidth([UIScreen mainScreen].bounds), CGRectGetHeight(self.keyboardFrame));
                    width = CGRectGetWidth([UIScreen mainScreen].bounds);
                    break;
                case UIDeviceOrientationLandscapeLeft:
                    // 注意，iOS7及以前，横屏时，键盘的height取到的是宽，width取到的是高；
                    srcPoint = CGPointMake(CGRectGetWidth(self.keyboardFrame), 0);
                    width = CGRectGetHeight([UIScreen mainScreen].bounds);
                    break;
                case UIDeviceOrientationLandscapeRight:
                    srcPoint = CGPointMake(CGRectGetWidth([UIScreen mainScreen].bounds) - CGRectGetWidth(self.keyboardFrame), CGRectGetHeight([UIScreen mainScreen].bounds));
                    width = CGRectGetHeight([UIScreen mainScreen].bounds);
                    break;
                default:
                    break;
